...
func TestHTTPSIngress(t *testing.T) {
	...
	secrets := []*corev1.Secret{
		...
		},
		{
			...
			Data: map[string][]byte{
				"tls.crt": []byte(tlsPairBarExample.Cert),
				"tls.key": []byte(tlsPairBarExample.Key),
			},
		},
	}
	...
	assert.Eventually(t, func() bool {
		...
		if resp.StatusCode == http.StatusOK {
			...
			require.True(t, n > 0)
			return strings.Contains(b.String(), "<title>httpbin.org</title>") && resp.TLS.PeerCertificates[0].Subject.CommonName == "foo.example"
		}
		return false
	...
	assert.Eventually(t, func() bool {
		...
		if resp.StatusCode == http.StatusOK {
			...
			require.NoError(t, err)
			require.True(t, n > 0)
			return strings.Contains(b.String(), "<title>httpbin.org</title>") && resp.TLS.PeerCertificates[0].Subject.CommonName == "bar.example"
		}
		return false
	}, ingressWait, waitTick, true)
