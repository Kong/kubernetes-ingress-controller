...
- A new gated feature `HashedDataplaneRouteNames` has been added. When enabled
  this feature causes the Kong Routes that are generated by the KIC to use a
  new naming convention that is unique and deterministic and avoids
  configuration thrash that could occur with the previous naming convention
  in situations where API resources (such as `Ingress`) are updated often.
  NOTE: if you enable this gated feature the route names captured in KIC
  prometheus metrics will change, so if you're parsing those you'll may need to
  update your logic to accomodate.
  [#834](https://github.com/Kong/kubernetes-ingress-controller/issues/834)
