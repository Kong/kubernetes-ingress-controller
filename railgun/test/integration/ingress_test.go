...
func TestHTTPSIngress(t *testing.T) {
	...
	testTransport := http.Transport{
		DialContext: func(ctx context.Context, network, addr string) (net.Conn, error) {
			if addr == "foo.example:443" {
				addr = fmt.Sprintf("%s:443", p.ProxyIP)
			}
	...
	t.Logf("creating an ingress for service %s with ingress.class %s", service.Name, ingressClass)
	ingress1 := k8sgen.NewIngressForService("/foo", map[string]string{
		annotations.IngressClassKey: ingressClass,
		"konghq.com/strip-path":     "true",
	}, service)
	...
	ingress2 := k8sgen.NewIngressForService("/bar", map[string]string{
		annotations.IngressClassKey: ingressClass,
		"konghq.com/strip-path":     "true",
	}, service)
	...
	secret1, err := cluster.Client().CoreV1().Secrets(corev1.NamespaceDefault).Create(ctx, secrets[0], metav1.CreateOptions{})
	assert.NoError(t, err)
	secret2, err := cluster.Client().CoreV1().Secrets(corev1.NamespaceDefault).Create(ctx, secrets[1], metav1.CreateOptions{})
	assert.NoError(t, err)
	ingress1, err = cluster.Client().NetworkingV1().Ingresses(corev1.NamespaceDefault).Create(ctx, ingress1, metav1.CreateOptions{})
	assert.NoError(t, err)
	ingress2, err = cluster.Client().NetworkingV1().Ingresses(corev1.NamespaceDefault).Create(ctx, ingress2, metav1.CreateOptions{})
	assert.NoError(t, err)

	defer func() {
		t.Logf("ensuring that Ingress %s is cleaned up", ingress1.Name)
		if err := cluster.Client().NetworkingV1().Ingresses(corev1.NamespaceDefault).Delete(ctx, ingress1.Name, metav1.DeleteOptions{}); err != nil {
			if !errors.IsNotFound(err) {
				require.NoError(t, err)
			}
		}
		t.Logf("ensuring that Ingress %s is cleaned up", ingress2.Name)
		if err := cluster.Client().NetworkingV1().Ingresses(corev1.NamespaceDefault).Delete(ctx, ingress2.Name, metav1.DeleteOptions{}); err != nil {
			if !errors.IsNotFound(err) {
				require.NoError(t, err)
			}
		}
		t.Logf("ensuring that Secret %s is cleaned up", secret1.Name)
		if err := cluster.Client().CoreV1().Secrets(corev1.NamespaceDefault).Delete(ctx, secret1.Name, metav1.DeleteOptions{}); err != nil {
			if !errors.IsNotFound(err) {
				require.NoError(t, err)
			}
		}
		t.Logf("ensuring that Secret %s is cleaned up", secret2.Name)
		if err := cluster.Client().CoreV1().Secrets(corev1.NamespaceDefault).Delete(ctx, secret2.Name, metav1.DeleteOptions{}); err != nil {
			if !errors.IsNotFound(err) {
				require.NoError(t, err)
			}
		}
	...
	// This should work currently. k8sgen.NewIngressForService() only creates path rules by default, so while we don't
	...
	ingress2, err = cluster.Client().NetworkingV1().Ingresses(corev1.NamespaceDefault).Get(ctx, ingress2.Name, metav1.GetOptions{})
	assert.NoError(t, err)
	ingress2.ObjectMeta.Annotations["konghq.com/snis"] = "bar.example"
	ingress2, err = cluster.Client().NetworkingV1().Ingresses(corev1.NamespaceDefault).Update(ctx, ingress2, metav1.UpdateOptions{})
	assert.NoError(t, err)
