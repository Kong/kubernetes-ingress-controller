...
type KongIngress struct {
	...
	// incoming requests over multiple services (targets).
	Upstream *KongIngressUpstream `json:"upstream,omitempty"`
	// Proxy is the abstraction of each of your own upstream services.
...
type KongIngressService struct {
	// The protocol used to communicate with the upstream.(default:"http")
...
type KongIngressUpstream struct {
	// HostHeaser is The hostname to be used as Host header
	...
	// Accepted values are: "none", "consumer", "ip", "header", "cookie". Default: "none".
