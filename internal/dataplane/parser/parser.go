...
func NewFeatureFlags(
	...
	if featureGates.Enabled(featuregates.CombinedRoutesFeature) {
		logger.V(util.InfoLevel).Info("combined routes mode has been enabled")
	}
...
func shouldEnableParserExpressionRoutes(
	...
	if !featureGates.Enabled(featuregates.CombinedRoutesFeature) {
		logger.V(util.InfoLevel).Info("ExpressionRoutes feature gate is enabled but CombinedRoutes feature gate is disabled, do not enable expression routes")
		return false
	}
