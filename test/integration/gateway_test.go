...
func TestUnmanagedGatewayControllerSupport(t *testing.T) {
	...
	for timeout.After(time.Now()) {
		...
		require.Equal(t, string(gatewayv1beta1.GatewayReasonNotReconciled), unsupportedGateway.Status.Conditions[0].Reason) //nolint:staticcheck
	}
}
...
func TestUnmanagedGatewayClass(t *testing.T) {
	...
	for timeout.After(time.Now()) {
		...
		require.Equal(t, string(gatewayv1beta1.GatewayReasonNotReconciled), gateway.Status.Conditions[0].Reason) //nolint:staticcheck
	}
