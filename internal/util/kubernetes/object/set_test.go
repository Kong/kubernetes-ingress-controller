...
func TestSet(t *testing.T) {
	...
	set := &Set{}
	require.False(t, set.Get(ing1))
	require.False(t, set.Get(ing2))
	require.False(t, set.Get(ing3))
	require.False(t, set.Get(tcp))

	t.Log("verifying object set insertion")
	set.InsertSucceeded(ing1)
	require.True(t, set.Get(ing1))
	require.False(t, set.Get(ing2))
	require.False(t, set.Get(ing3))
	require.False(t, set.Get(tcp))
	set.InsertSucceeded(ing2)
	require.True(t, set.Get(ing1))
	require.True(t, set.Get(ing2))
	require.False(t, set.Get(ing3))
	require.False(t, set.Get(tcp))
	set.InsertSucceeded(ing3)
	require.True(t, set.Get(ing1))
	require.True(t, set.Get(ing2))
	require.True(t, set.Get(ing3))
	require.False(t, set.Get(tcp))
	set.InsertSucceeded(tcp)
	require.True(t, set.Get(ing1))
	require.True(t, set.Get(ing2))
	require.True(t, set.Get(ing3))
	require.True(t, set.Get(tcp))
}
