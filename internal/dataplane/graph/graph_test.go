...
- algorithm: round-robin
  ...
  - k8s-version:v1`
	...
	// Entities errors can be either returned from the Parser (we can detect some errors proactively while parsing e.g.
	// two plugins of the same type on the same entity) or from Kong itself (flattened errors).
