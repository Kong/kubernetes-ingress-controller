...
//const testPerfTCPIngressNamespace = "tcpingress"
const testPerfTCPIngressNamespace = "tcpingress-1"
...
func TestTCPPerf(t *testing.T) {
	...
	t.Logf("creating namespace %s for testing TCPIngress", testPerfTCPIngressNamespace)
	ns := &corev1.Namespace{ObjectMeta: metav1.ObjectMeta{Name: testPerfTCPIngressNamespace}}
	ns, err = cluster.Client().CoreV1().Namespaces().Create(ctx, ns, metav1.CreateOptions{})
	require.NoError(t, err)

	// defer func() {
	// 	t.Logf("cleaning up namespace %s", testPerfTCPIngressNamespace)
	...
	deployment, err = cluster.Client().AppsV1().Deployments(testPerfTCPIngressNamespace).Create(ctx, deployment, metav1.CreateOptions{})
	require.NoError(t, err)

	// defer func() {
	// 	t.Logf("cleaning up the deployment %s", deployment.Name)
	// 	assert.NoError(t, cluster.Client().AppsV1().Deployments(testPerfTCPIngressNamespace).Delete(ctx, deployment.Name, metav1.DeleteOptions{}))
	// }()

	t.Logf("exposing deployment %s via service", deployment.Name)
	service := k8sgen.NewServiceForDeployment(deployment, corev1.ServiceTypeLoadBalancer)
	service, err = cluster.Client().CoreV1().Services(testPerfTCPIngressNamespace).Create(ctx, service, metav1.CreateOptions{})
	require.NoError(t, err)

	// defer func() {
	// 	t.Logf("cleaning up the service %s", service.Name)
	// 	assert.NoError(t, cluster.Client().CoreV1().Services(testPerfTCPIngressNamespace).Delete(ctx, service.Name, metav1.DeleteOptions{}))
	// }()
	...
	tcp := &kongv1beta1.TCPIngress{
		ObjectMeta: metav1.ObjectMeta{
			Name:      testName,
			Namespace: testPerfTCPIngressNamespace,
	...
	tcp, err = c.ConfigurationV1beta1().TCPIngresses(testPerfTCPIngressNamespace).Create(ctx, tcp, metav1.CreateOptions{})
	require.NoError(t, err)

	// defer func() {
	// 	t.Logf("ensuring that TCPIngress %s is cleaned up", tcp.Name)
	// 	if err := c.ConfigurationV1beta1().TCPIngresses(testPerfTCPIngressNamespace).Delete(ctx, tcp.Name, metav1.DeleteOptions{}); err != nil {
	...
	// require.NoError(t, c.ConfigurationV1beta1().TCPIngresses(testPerfTCPIngressNamespace).Delete(ctx, tcp.Name, metav1.DeleteOptions{}))
	// require.Eventually(t, func() bool {
	// 	resp, err := httpc.Get(tcpProxyURL.String())
	// 	if err != nil {
	// 		return true
