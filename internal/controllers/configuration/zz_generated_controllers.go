...
func (r *KongV1Alpha1KongCustomEntityReconciler) SetupWithManager(mgr ctrl.Manager) error {
	c, err := controller.New("KongV1Alpha1KongCustomEntity", mgr, controller.Options{
		Reconciler: r,
		LogConstructor: func(_ *reconcile.Request) logr.Logger {
			return r.Log
		},
	...
	preds := ctrlutils.GeneratePredicateFuncsForIngressClassFilter(r.IngressClassName)
	return c.Watch(
		source.Kind(mgr.GetCache(), &kongv1alpha1.KongCustomEntity{}),
		&handler.EnqueueRequestForObject{},
		preds,
	)
}
...
func (r *KongV1Alpha1KongCustomEntityReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	...
	if err := r.Get(ctx, req.NamespacedName, obj); err != nil {
		if apierrors.IsNotFound(err) {
			...
			// remove reference record where the KongCustomEntity is the referrer
			if err := ctrlref.DeleteReferencesByReferrer(r.ReferenceIndexers, r.DataplaneClient, obj); err != nil {
				return ctrl.Result{}, err
			}
	...
	// update reference relationship from the KongCustomEntity to other objects.
	if err := updateReferredObjects(ctx, r.Client, r.ReferenceIndexers, r.DataplaneClient, obj); err != nil {
		if apierrors.IsNotFound(err) {
			// reconcile again if the secret does not exist yet
			return ctrl.Result{
...
// -----------------------------------------------------------------------------
// KongV1Alpha1 KongCustomEntityDefinition - Reconciler
// -----------------------------------------------------------------------------

// KongV1Alpha1KongCustomEntityDefinitionReconciler reconciles KongCustomEntityDefinition resources
type KongV1Alpha1KongCustomEntityDefinitionReconciler struct {
	client.Client
...
var _ controllers.Reconciler = &KongV1Alpha1KongCustomEntityDefinitionReconciler{}

// SetupWithManager sets up the controller with the Manager.
func (r *KongV1Alpha1KongCustomEntityDefinitionReconciler) SetupWithManager(mgr ctrl.Manager) error {
	c, err := controller.New("KongV1Alpha1KongCustomEntityDefinition", mgr, controller.Options{
		Reconciler: r,
		LogConstructor: func(_ *reconcile.Request) logr.Logger {
			return r.Log
		},
	...
	return c.Watch(
		source.Kind(mgr.GetCache(), &kongv1alpha1.KongCustomEntityDefinition{}),
		&handler.EnqueueRequestForObject{},
	)
}

// SetLogger sets the logger.
func (r *KongV1Alpha1KongCustomEntityDefinitionReconciler) SetLogger(l logr.Logger) {
	r.Log = l
}

//+kubebuilder:rbac:groups=configuration.konghq.com,resources=kongcustomentitydefinitions,verbs=get;list;watch

// Reconcile processes the watched objects
func (r *KongV1Alpha1KongCustomEntityDefinitionReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	log := r.Log.WithValues("KongV1Alpha1KongCustomEntityDefinition", req.NamespacedName)

	// get the relevant object
	obj := new(kongv1alpha1.KongCustomEntityDefinition)
	// set type meta to the object
	obj.TypeMeta = metav1.TypeMeta{
		APIVersion: kongv1alpha1.SchemeGroupVersion.String(),
	...
	// clean the object up if it's being deleted
	if !obj.DeletionTimestamp.IsZero() && time.Now().After(obj.DeletionTimestamp.Time) {
		log.V(util.DebugLevel).Info("resource is being deleted, its configuration will be removed", "type", "KongCustomEntityDefinition", "namespace", req.Namespace, "name", req.Name)
