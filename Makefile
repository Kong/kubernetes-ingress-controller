...
test.integration.dbless:
	...
	@TEST_DATABASE_MODE="off" USE_ENDPOINT_SLICES="true" GOFLAGS="-tags=integration_tests" go test -timeout 15m -race -v -count=1 -covermode=atomic -coverpkg=$(PKG_LIST) -coverprofile=$(COVERAGE_INTEGRATION_PROFILE) ./test/integration/
