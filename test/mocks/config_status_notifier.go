...
func (c *ConfigStatusNotifier) LastKonnectConfigStatus() (clients.KonnectConfigUploadStatus, bool) {
	c.lock.RLock()
	defer c.lock.RUnlock()
	if len(c.receivedKonnectConfigStatus) == 0 {
		return clients.KonnectConfigUploadStatus{}, false
	}
	return c.receivedKonnectConfigStatus[len(c.receivedKonnectConfigStatus)-1], true
}

func (c *ConfigStatusNotifier) KonnectConfigStatuses() []clients.KonnectConfigUploadStatus {
	c.lock.RLock()
	defer c.lock.RUnlock()
	copied := make([]clients.KonnectConfigUploadStatus, len(c.receivedKonnectConfigStatus))
	copy(copied, c.receivedKonnectConfigStatus)
	return copied
}
