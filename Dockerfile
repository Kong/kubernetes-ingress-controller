...
# Build
RUN CGO_ENABLED=0 GOOS=linux GOARCH=amd64 GO111MODULE=on go build -a -o manager -ldflags "-s -w -X github.com/kong/kubernetes-ingress-controller/v2/internal/manager.Release=$TAG -X github.com/kong/kubernetes-ingress-controller/v2/internal/manager.Commit=$COMMIT -X github.com/kong/kubernetes-ingress-controller/v2/internal/manager.Repo=$REPO_INFO" ./internal/cmd/main.go
